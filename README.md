...
| `docs/xAgent_Foundry构建部署与测试培训手册_v1.0.md` | 培训讲义和实验步骤 |
...
| `scripts/get-training-context.ps1` | 自动发现当前 azd/Azure 环境资源名称 |
...
| `scripts/requirements-docs.txt` | 生成培训 PDF 所需的独立 Python 依赖 |
...
`get-training-context.ps1` 只读取当前 azd 环境和目标资源组。订阅 ID、资源 ID 和 endpoint 仅用于本地操作，不应粘贴到公开材料。
...
性能、安全、Portal/CLI 评估、追踪、监控、AI 红队测试与防护栏的完整步骤见两份培训手册。
...
.\.venv-docs\Scripts\python.exe .\scripts\render_training_docs.py .\docs\xAgent_Foundry构建部署与测试培训手册_v1.0.md .\docs\xAgent_Foundry性能安全与Guardrails实验手册_v1.0.md
```
...
Windows 深层工作区可能触发 Python 长路径问题。培训现场建议将代码放在较短路径，例如 `C:\labs\xagent`。如果不能移动，可临时使用 `subst` 映射短盘符。
